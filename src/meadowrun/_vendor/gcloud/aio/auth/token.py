...
class Token:
    ...
    def __init__(
        ...
        if self.token_type == Type.SERVICE_ACCOUNT and not self.scopes:
            raise Exception(
                "scopes must be provided when token type is " "service account"
            )
        ...
        self.acquiring: Optional[
            asyncio.Future[Any]
        ] = None  # pylint: disable=unsubscriptable-object,line-too-long
    ...
    async def get_project(self) -> Optional[str]:
        ...
        if self.token_type == Type.GCE_METADATA:
            ...
            resp = await self.session.get(
                GCE_ENDPOINT_PROJECT, timeout=10, headers=GCE_METADATA_HEADERS
            )
    ...
    async def _refresh_authorized_user(self, timeout: int) -> Response:
        ...
        data: Response = await self.session.post(
            url=self.token_uri, data=payload, headers=REFRESH_HEADERS, timeout=timeout
        )
        return data

    async def _refresh_gce_metadata(self, timeout: int) -> Response:
        resp: Response = await self.session.get(
    ...
    async def _refresh_service_account(self, timeout: int) -> Response:
        ...
        assertion = jwt.encode(
            assertion_payload, self.service_data["private_key"], algorithm="RS256"
        )
        payload = urlencode(
            {
                "assertion": assertion,
        ...
        resp: Response = await self.session.post(
