...
class BaseSession:
    ...
    @abstractmethod
    async def put(
        self, url: str, headers: Dict[str, str], data: IO[Any], timeout: float
    ) -> Response:
        pass
    ...
    class AioSession(BaseSession):
        ...
        async def post(
            ...
            resp = await self.session.post(
                url, data=data, headers=headers, timeout=timeout, params=params
            )
...
if BUILD_GCLOUD_REST:
    ...
    class SyncSession(BaseSession):
        ...
        async def get(
            ...
            with self.google_api_lock:
                resp = self.session.get(
                    url, headers=headers, timeout=timeout, params=params, stream=stream
                )
            resp.raise_for_status()
            return resp
        ...
        async def put(
            self, url: str, headers: Dict[str, str], data: IO[Any], timeout: float = 10
        ) -> Response:
            with self.google_api_lock:
