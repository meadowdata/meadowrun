...
@dataclasses.dataclass(frozen=True)
class CondaEnvironmentFile(InterpreterSpecFile):
    """
    Specifies a file that can be used to create a conda environment. The file can either
    be in the format generated by `conda env export` (often referred to as
    environment.yml files), or by `conda list --explicit`.

    Attributes:
        path_to_file: In the context of mirror_local, this is the path to a file on the
            local disk. In the context of git_repo, this is a path to a file in the git
            repo
        file_format: "env_export" specifies the `conda env export` format. "list"
            specifies the `conda list --explicit` format. None will auto-detect between
            these two formats
    ...
    path_to_file: str
    file_format: Literal[None, "env_export", "list"] = None
    additional_software: Union[Sequence[str], str, None] = None

    def file_format_enum(self) -> EnvironmentFileFormat.ValueType:
        if self.file_format is None:
            return EnvironmentFileFormat.ENV_FILE_FORMAT_DEFAULT
        elif self.file_format == "env_export":
            return EnvironmentFileFormat.ENV_FILE_FORMAT_CONDA_ENV_EXPORT
        elif self.file_format == "list":
            return EnvironmentFileFormat.ENV_FILE_FORMAT_CONDA_LIST
        else:
            raise ValueError(f"file_format was not valid {self.file_format}")


def CondaEnvironmentYmlFile(
    path_to_yml_file: str, additional_software: Union[Sequence[str], str, None] = None
) -> CondaEnvironmentFile:
    """
    Only exists for backwards compatibility. Please switch to using CondaEnvironmentFile
    directly
    """
    return CondaEnvironmentFile(
        path_to_yml_file, additional_software=additional_software
    )
    ...
    async def get_interpreter_spec(self) -> InterpreterDeployment:
        ...
        if conda_env_spec is not None:
            ...
            return EnvironmentSpec(
                ...
                file_format=EnvironmentFileFormat.ENV_FILE_FORMAT_CONDA_LIST,
                spec=conda_env_spec,
...
class Deployment:
    ...
    async def mirror_local(
        ...
        if isinstance(interpreter, CondaEnvironmentFile):
            with open(interpreter.path_to_file, encoding="utf-8") as f:
    ...
    def git_repo(
        ...
        elif isinstance(interpreter, CondaEnvironmentFile):
            interpreter_spec = EnvironmentSpecInCode(
                environment_type=EnvironmentType.ENV_TYPE_CONDA,
                path_to_spec=interpreter.path_to_file,
                additional_software=_additional_software_to_dict(
                    interpreter.additional_software
                ),
