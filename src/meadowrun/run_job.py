...
class Deployment:
    ...
    def git_repo(
        ...
        - path_in_repo: e.g. "src/python" to use a subdirectory of the repo
        - conda_yml_file: a directory (relative to the repo, or path_in_repo if that's
          specified) that points to a file generated by `conda env export`. This file
          will be used to generate the environment to run in.
        ...
        if branch:
            code: Union[CodeDeployment, VersionedCodeDeployment, None] = GitRepoBranch(
                repo_url=repo_url,
                branch=branch,
                # the generated protobuf types say that path_in_repo must not be None,
                # but this is not actually the case, it handles None fine, which we want
                # to take advantage of
                path_in_repo=cast(str, path_in_repo),
            )
        else:  # commit
