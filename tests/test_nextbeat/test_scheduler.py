...
def launch(*_args: Any, **_kwargs: Any) -> bool:
    return True
...
def test_scheduling_sequential_jobs() -> None:
    ...
    scheduler.add_job(Job("A", launch, ()))
    trigger_action = (JobStateChangeTrigger("A", ("succeeded", "failed")), Actions.run)
    scheduler.add_job(Job("B", launch, (trigger_action,)))
...
def test_scheduling_join() -> None:
    ...
    scheduler.add_job(Job("A", launch, (), args=["A"]))
    scheduler.add_job(Job("B", launch, (), args=["B"]))
    ...
    scheduler.add_job(Job("C", launch, (trigger_action,), args=["C"]))
    scheduler.update_subscriptions()
