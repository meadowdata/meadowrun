...
Runnable = Union[MeadowRunCommand, MeadowRunFunction]
...
@dataclasses.dataclass(frozen=True)
class MeadowRunDeployedRunnable:
    """
    A "runnable" that a MeadowRun agent can run. A runnable can either be a python
    function or a command line. Specifies deployments that tell a MeadowRun agent where
...
@dataclasses.dataclass(frozen=True)
class MeadowRunVersionedDeployedRunnable:
    """
    Like MeadowRunDeployedRunnable, but code_deployment and interpreter_deployment can
    ...
    async def get_latest(
        ...
        return MeadowRunDeployedRunnable(
            code_deployment,
